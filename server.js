...
app.post('/api/farms', (req, res) => {
  const { farm_name, contact_name, email, phone, country, region,
          traceability_id, farm_type, calving_season, non_replacements_available, notes } = req.body;
  const errors = [];
...
app.post('/api/farms/:id/animals/upload', upload.single('csv'), (req, res) => {
  ...
  res.json({ success: true, summary: { total_rows: parseResult.total, inserted, duplicates, rejected: parseResult.rejected, warnings: parseResult.results.filter(r => r.warnings && r.warnings.length > 0).length }, errors: errorDetails.length > 0 ? errorDetails : undefined });
});
...
app.get('/api/farms/:id/animals', (req, res) => {
  ...
  const stats = db.prepare("SELECT COUNT(*) as total, SUM(CASE WHEN sex = 'bull' THEN 1 ELSE 0 END) as bulls, SUM(CASE WHEN sex = 'steer' THEN 1 ELSE 0 END) as steers, SUM(CASE WHEN sex = 'heifer' THEN 1 ELSE 0 END) as heifers, AVG(birth_weight_kg) as avg_birth_weight, MIN(birth_date) as earliest_birth, MAX(birth_date) as latest_birth FROM animals WHERE farm_id = ?").get(req.params.id);
...
app.post('/api/farms', (req, res) => {
  ...
  try {
    const stmt = db.prepare(`
      INSERT INTO farms (farm_name, contact_name, email, phone, country, region,
        traceability_id, farm_type, calving_season, non_replacements_available, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);

    const result = stmt.run(
      farm_name, contact_name, email, phone || null, country, region,
      traceability_id, farm_type, calving_season || null,
      non_replacements_available ? parseInt(non_replacements_available) : null,
      notes || null
    );

    res.json({
      success: true,
      farm_id: result.lastInsertRowid,
      message: `Farm "${farm_name}" registered successfully. Your farm ID is ${result.lastInsertRowid}.`
    });
  } catch (err) {
    if (err.message.includes('UNIQUE')) {
      return res.status(409).json({ success: false, errors: ['A farm with this traceability ID already exists'] });
    }
    console.error('Farm registration error:', err);
    res.status(500).json({ success: false, errors: ['Internal server error'] });
  }
...
app.post('/api/farms/:id/animals/upload', upload.single('csv'), (req, res) => {
  ...
  // Insert valid animals
  const insertAnimal = db.prepare(`
    INSERT OR IGNORE INTO animals (farm_id, eid, vid, sex, breed, birth_date, birth_weight_kg,
      birth_farm_id, dam_eid, sire_breed, collection_date, collection_weight_kg, notes)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertMany = db.transaction((rows) => {
    for (const result of rows) {
      ...
      const d = result.data;
      try {
        const r = insertAnimal.run(
          farm.id, d.eid, d.vid, d.sex, d.breed, d.birth_date, d.birth_weight_kg,
          d.birth_farm_id, d.dam_eid, d.sire_breed, d.collection_date, d.collection_weight_kg, d.notes
        );
        if (r.changes > 0) inserted++;
        else duplicates++;
      } catch (err) {
  ...
  // Log the upload
  const errorDetails = parseResult.results
    .filter(r => !r.valid)
    .map(r => ({ row: r.row, errors: r.errors }));

  db.prepare(`
    INSERT INTO csv_uploads (farm_id, filename, upload_type, rows_total, rows_accepted, rows_rejected, errors)
    VALUES (?, ?, 'animal_registration', ?, ?, ?, ?)
  `).run(
    farm.id, req.file.originalname, parseResult.total,
    inserted, parseResult.rejected + duplicates,
  ...
  res.json({
    ...
    summary: {
      ...
      warnings: parseResult.results.filter(r => r.warnings && r.warnings.length > 0).length
    },
    errors: errorDetails.length > 0 ? errorDetails : undefined,
    warnings: parseResult.results
      .filter(r => r.warnings && r.warnings.length > 0)
      .map(r => ({ row: r.row, warnings: r.warnings }))
  });
});

// --- Get Animals for Farm ---
app.get('/api/farms/:id/animals', (req, res) => {
  const farm = db.prepare('SELECT * FROM farms WHERE id = ?').get(req.params.id);
  if (!farm) return res.status(404).json({ success: false, error: 'Farm not found' });
  ...
  const animals = db.prepare(`
    SELECT a.*, m.mob_name
    FROM animals a
    LEFT JOIN mobs m ON a.mob_id = m.id
    WHERE a.farm_id = ?
  ...
  const stats = db.prepare(`
    SELECT
      COUNT(*) as total,
      SUM(CASE WHEN sex = 'bull' THEN 1 ELSE 0 END) as bulls,
      SUM(CASE WHEN sex = 'steer' THEN 1 ELSE 0 END) as steers,
      SUM(CASE WHEN sex = 'heifer' THEN 1 ELSE 0 END) as heifers,
...
app.get('/api/farms', (req, res) => {
  const farms = db.prepare(`
    SELECT f.*,
      (SELECT COUNT(*) FROM animals WHERE farm_id = f.id) as animal_count,
      (SELECT COUNT(*) FROM mobs WHERE farm_id = f.id) as mob_count
    FROM farms f
...
app.get('/api/stats', (req, res) => {
  ...
  const nzFarms = db.prepare("SELECT COUNT(*) as count FROM farms WHERE country = 'NZ'").get();
  const auFarms = db.prepare("SELECT COUNT(*) as count FROM farms WHERE country = 'AU'").get();
  const recentUploads = db.prepare('SELECT COUNT(*) as count FROM csv_uploads WHERE created_at > datetime("now", "-7 days")').get();

  res.json({
    success: true,
    stats: {
      total_farms: farmCount.count,
      total_animals: animalCount.count,
      nz_farms: nzFarms.count,
      au_farms: auFarms.count,
      uploads_this_week: recentUploads.count
    }
  });
});
